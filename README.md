...
2. **Sinh (Generation):** đưa các đoạn đó cho Claude và yêu cầu trả lời *dựa trên chúng*.
...
LÚC HỎI (chạy mỗi câu hỏi):
  Câu hỏi
      │  embeddings.py  → biến câu hỏi thành vector
      ▼
  Vector câu hỏi
      │  vector_store.py→ tìm TOP_K đoạn gần nghĩa nhất (📦)
      ▼
  Vài đoạn liên quan
      │  rag.py         → ghép thành prompt + gọi Claude
      ▼
  ✅ Câu trả lời + danh sách nguồn
```
...
│  ├─ ingestion.py           # "nhạc trưởng": ghép 4 bước nạp tài liệu
│  ├─ rag.py                 # TRUY HỒI + gọi Claude → câu trả lời
│  └─ main.py                # FastAPI: các API + phục vụ web
├─ web/
...
Trong `rag.py`, system prompt buộc Claude:
...
## 5b. Chế độ Agentic RAG (bật sẵn)

Mặc định `RAG_MODE=agentic`. Thay vì tìm 1 lần, hệ thống **tự điều hướng việc tìm kiếm**:
...
- **Tăng độ chính xác truy hồi:** thêm bước *re-ranking* (xếp hạng lại top-K), hoặc
  tăng `TOP_K` rồi cho Claude tự lọc.
